...
後端Node.js + Express + Axios
AI引擎Google Gemini 1.5 Flash
...
病人端輸入用藥描述文字後，送至後端路由 /api/interaction/analyze。
後端呼叫 Gemini AI 進行風險分析，取得結構化結果。
分析結果透過 fhirService.js 儲存為 FHIR Observation 資源（使用病人 ID 131431142）。
醫師端輸入病人 ID 後，送至後端路由 /cds-services/interaction-check。
後端先呼叫 FHIR Server 讀取該病人的歷史 Observation 記錄。
若有歷史記錄，則直接回傳歷史警示卡片給醫師端。
若無歷史記錄，則再次呼叫 Gemini AI 分析新處方藥物。
最終將結果轉換為 CDS Hooks 格式的卡片，回傳給醫師端前端顯示。

此設計實現了「一次分析、多端共用」的資料流閉環。
